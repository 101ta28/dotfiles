...
The installer also links `.config/ghostty/config.ghostty` to `~/.config/ghostty/config.ghostty` with the same existing-file policy. Ghostty itself is not installed. Non-color settings are based on GNOME Terminal profile `b1dcc9dd-5262-4d8d-a863-c897e6d979b9`: blinking block cursor, scrolling behavior, system bell, and closing after command exit. The font is customized to HackGen Console NF (13 pt), and the initial size is set to 134 columns × 34 rows to approximate the GNOME Terminal window size on this desktop. Colors remain Ghostty defaults. Settings without an exact equivalent, such as the 10,000-line scrollback and always-visible scrollbar, retain Ghostty defaults. This is a snapshot; edit the repository file to change the settings. GTK window decorations can cause the actual grid size to differ from the requested initial size.
